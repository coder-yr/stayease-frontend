...
const Navbar: React.FC = () => {
  ...
  const handleLogout = () => {
    ...
    const authKeys = ["accessToken", "token", "authToken", "jwt", "session", "auth", "user", "stayease_auth"];
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex items-center gap-3">
                <Link 
                  to="/signup" 
                  className="text-[10px] font-bold text-brand-primary uppercase tracking-widest hover:text-brand-accent transition-colors px-6 py-2.5 rounded-full bg-brand-accent/10 border border-brand-accent/20"
                >
                  Sign Up
                </Link>
                <Link 
              ...
              {isAuthenticated ? (
                ...
                <div className="flex flex-col gap-3">
                  <Link
                    to="/signup"
                    onClick={() => setIsOpen(false)}
                    className="w-full flex items-center justify-center p-6 rounded-[2rem] bg-[#10b981] text-white font-bold uppercase tracking-widest shadow-2xl shadow-emerald-500/10"
                  >
                    Create Account
                  </Link>
                  <Link
