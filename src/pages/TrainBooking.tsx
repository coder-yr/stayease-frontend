...
import { Train as TrainIcon, CreditCard, User, ShieldCheck, ArrowRight, MapPin, Calendar, Clock } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { bookingApi } from '../services/bookingApi';
import { Train } from '../services/trainApi';

const TrainBooking: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state as { train?: Train, travelers?: number } | undefined;
  ...
  const totalAmount = Number(train.price) * travelers;
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const payload = {
        type: 'train',
        travelDate: new Date(train.departureTime).toISOString(),
        totalAmount,
    ...
    <div className="min-h-screen bg-brand-bg pt-24 pb-32">
      <div className="max-w-5xl mx-auto px-6 space-y-12">
        ...
        <div className="relative overflow-hidden bg-brand-primary rounded-[48px] p-10 md:p-16 text-white shadow-2xl">
          <TrainIcon className="absolute -right-20 -bottom-20 w-80 h-80 text-white/5 rotate-12" />
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
            <div className="space-y-6 max-w-2xl">
              <span className="micro-label text-brand-accent-light block">Reservation Portal</span>
              <h1 className="editorial-title text-5xl md:text-7xl leading-tight">
                Secure Your <span className="serif-italic editorial-skew inline-block text-brand-accent-light">Journey</span>
              </h1>
              <p className="font-serif italic text-xl text-white/60">
                Confirm your details to finalize your luxury rail experience.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
              ...
              {step === 1 ? (
                ...
                >
                  <div className="space-y-2">
                    <h3 className="text-3xl font-display font-bold text-brand-primary">Passenger <span className="serif-italic text-brand-accent">Details</span></h3>
                    <p className="text-slate-500 text-sm">Please provide accurate identification details as they appear on your ID.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-3">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50/50 border border-slate-100 rounded-3xl py-5 px-8 focus:bg-white focus:ring-2 focus:ring-brand-accent/20 focus:border-brand-accent/50 outline-none font-bold text-brand-primary transition-all"
                        placeholder="john.doe@reside.com"
                      />
                    </div>
                    <div className="space-y-3">
                ...
                >
                  ...
                  <div className="bg-brand-accent/5 border border-brand-accent/20 rounded-[32px] p-8 flex items-start gap-6">
                    ...
                    <div className="space-y-1">
                      <h4 className="font-display font-bold text-brand-primary tracking-tight">StayEase Payment Shield</h4>
                      <p className="text-sm text-slate-600">Your transaction is protected by bank-grade 256-bit encryption and multi-layer authentication.</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 pt-4">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center text-slate-600 font-medium pb-4 border-b border-slate-100">
                        ...
                        <span>₹{Number(train.price).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center text-slate-600 font-medium pb-4 border-b border-slate-100">
                  ...
                  >
                    {isSubmitting ? "PROCESSING BOOKING..." : `BOOK NOW · ₹${totalAmount.toLocaleString()}`}
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="space-y-8">
            <div className="glass-card p-8 rounded-[40px] space-y-8">
              <h4 className="text-sm uppercase tracking-widest text-slate-400 font-bold border-b border-slate-100 pb-4">Itinerary Overview</h4>
              ...
              <div className="space-y-8">
                <div className="flex items-center gap-5">
                   ...
                   <div>
                     <div className="text-sm font-bold text-brand-primary uppercase tracking-tight leading-tight">{train.operator}</div>
                     <div className="text-[10px] font-bold text-slate-400 mt-0.5">{train.trainNumber || 'EXP-234'}</div>
                   </div>
                </div>
                ...
                <div className="relative pl-6 space-y-8 before:absolute before:left-[10px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gradient-to-b before:from-brand-accent before:to-transparent">
                  <div className="relative">
                    ...
                    <div className="text-sm font-bold text-brand-primary">{new Date(train.departureTime).toLocaleTimeString([], { hour: '2-digit', minute:'2-digit' })}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{train.source}</div>
                  </div>
                  <div className="relative">
                    <div className="absolute -left-[20px] top-1.5 w-[10px] h-[10px] rounded-full bg-slate-300" />
                    <div className="text-sm font-bold text-brand-primary">{new Date(train.arrivalTime).toLocaleTimeString([], { hour: '2-digit', minute:'2-digit' })}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{train.destination}</div>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                   <div className="space-y-1">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Class</span>
                      <span className="text-xs font-bold text-brand-primary">{train.classType || 'Sleeper'}</span>
                   </div>
                   <div className="space-y-1">
            ...
            <div className="glass-card p-8 rounded-[40px] bg-slate-800 text-white border-none shadow-brand-primary/20">
               <div className="flex items-center gap-4 mb-4">
                 <CreditCard className="w-6 h-6 text-brand-accent-light" />
                 <h5 className="font-display font-bold">Booking Perks</h5>
               </div>
               <ul className="space-y-3">
                 {['Complimentary Meal', 'Cancellable', 'Premium Cabin'].map(perk => (
                   <li key={perk} className="flex items-center gap-3 text-xs font-medium text-white/70 italic font-serif">
                     <span className="w-1 h-1 rounded-full bg-brand-accent-light" /> {perk}
                   </li>
                 ))}
               </ul>
            </div>
